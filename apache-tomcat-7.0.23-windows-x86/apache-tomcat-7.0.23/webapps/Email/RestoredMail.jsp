...
<tr>
	...
	<td WIDTH="10%" bgcolor="#CFECEC"><button name="Display1" style="background-color:transparent;border:0px" class="displayemailnormal" Onclick="javascript:loaddisplaymessagedraft(<%=i%>)"><%= (InternetAddress.toString(message.getFrom())) %></button></td>
...
if(count>=50) break;
	...
	<td><input type="button" class="button" name="first" value="First" onclick="javascript:loadFirstFifty('RestoredMail')"/></td>
	<td>
	<% 
	if(lastMailIndex<messages.length)
	{
	%>
	<input type="button" class="button" name="next" value="Next" onclick="javascript:loadnext('RestoredMail')"/>
	<%
	} 
	%>
	</td>
	<td>
	<%if(MailIndex>49){%>
	<input type="button" class="button" name="previous" value="Previous" onclick="javascript:loadprevious('RestoredMail')"/>
	<%}%>
	</td>
	<td>
	<input type="button" class="button" name="last" value="Last" onclick="javascript:loadLastFifty('RestoredMail')"/>
	</td>
</table>
